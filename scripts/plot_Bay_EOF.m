...
SST = TEMPv3(:,:,L7L8Dates);
LAT = LAT1; LON = LON1;
clear D_SST LAT1 LON1 TEMPv3
...
%% 5. Fill in with the nearest TEMPv3oral data 
sstr = fillmissing(sstr,'linear',1,'EndValues','near');
SST = sstr;
%SST = rmmissing(SST,2);

%% 6. Normalize the TEMPv3 data
...
figure(1), clf;
%[~, hContour]=contourf(LON,LAT,flip(finaleeof1s).*U1(1,1),75,'edgecolor','none');
pcolor(LON,LAT,flip(finaleeof1s).*U1(1,1));
set(gca,'Color',[0.5 0.5 0.5]);
shading interp;
colormap(gca,clri1);
caxis([5e-5 15e-5]); colorbar
...
nexttile
[~, hContour]=contourf(LON,LAT,flip(finaleeof2s).*U2(18,1),75,'edgecolor','none');
%pcolor(LON,LAT,flip(finaleeof2s).*U2(18,1));
set(gca,'Color',[0.5 0.5 0.5]);
shading interp
colormap(gca,clri1);
caxis([-1e-5 1e-5]);
...
% Plot the EOF3
nexttile
[~, hContour]=contourf(LON,LAT,flip(finaleeof3s).*U3(6,1),75,'edgecolor','none');
%pcolor(LON,LAT,flip(finaleeof3s).*U3(6,1));
set(gca,'Color',[0.5 0.5 0.5]);
shading interp
colormap(gca,clri1);
caxis([-1e-5 1e-5]);
(diag(S(2,2))+diag(S(1,1))+diag(S(3,3)))^2/(sum(diag(S)))^2
colormap(gca,[clri1]);
hold on; box on; 
set(gca, 'GridLineStyle', '-', 'MinorGridLineStyle', ':', 'LineWidth', 3);
set(gca, 'FontSize', 25, 'FontWeight', 'bold');
...
for i = 1:30
nexttile
[~, hContour]=contourf(LON,LAT,flip(finaleeof2s).*U2(i,1),50,'edgecolor','none');
...
nexttile
plot(TT.Var1,(-1).*U1,'color','#0000FF','markersize',1,'linewidth',2); hold on;
set(gca, 'XTickLabel');ylim([-0.2 0.2]);
hold on; box on; 
set(gca, 'GridLineStyle', '-', 'MinorGridLineStyle', ':', 'LineWidth', 3);
set(gca, 'FontSize', 15, 'FontWeight', 'bold'); ylabel('Normalized Unit');
set(gca, 'Layer', 'top'); legend('EOF1','location','northwest','fontsize',16);
xlabel('Year');
...
set(gca, 'GridLineStyle', '-', 'MinorGridLineStyle', ':', 'LineWidth', 3);
set(gca, 'FontSize', 15, 'FontWeight', 'bold'); ylabel('Normalized Unit');
set(gca, 'Layer', 'top'); legend('EOF2','location','northwest','fontsize',16);
xlabel('Year');
set(gcf, 'Position', [-1577,590,652,583]);
...
nexttile
loglog(f,pxx1,'color','#0000FF','linewidth',2); hold on
loglog(f,pxx2,'color','#A2142F','linewidth',2); hold on
...
set(gca, 'GridLineStyle', '-', 'MinorGridLineStyle', ':', 'LineWidth', 3);
set(gca, 'FontSize', 15, 'FontWeight', 'bold');
set(gca, 'Layer', 'top'); ylabel('Magnitude');
legend('EOF1: Power Spectrum','EOF2: Power Spectrum','location','northwest','fontsize',16);
toc