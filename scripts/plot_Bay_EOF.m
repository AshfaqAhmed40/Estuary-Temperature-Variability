...
while ~isempty(C)
    len = C(2, 1);
    plot(C(1, 2:1+len), C(2, 2:1+len), 'k', 'LineWidth', 2);
    C(:, 1:len+1) = []; hold on;
end
    
%[~, hContour]=contourf(flip(finaleeof1s).*U1(1,1),25, 'linewidth',1);
pcolor(flip(finaleeof1s).*U1(1,1));
set(gca,'Color',[0.5 0.5 0.5]);
shading interp;
colormap(gca, flip(slanCM(ColorMap)));
caxis([10e-5 12e-5]); colorbar
...
[~, hContour]=contourf(flip(finaleeof2s).*U2(18,1),75,'edgecolor','none');
...
[~, hContour]=contourf(flip(finaleeof3s).*U3(6,1),75,'edgecolor','none');
% pcolor(flip(finaleeof2s).*U2(18,1));
set(gca,'Color',[0.5 0.5 0.5]);
shading interp
caxis([-1e-5 1e-5]);
