...
for i = 1:size(Decade,2)
    nexttile
    pcolor(LON1,LAT1,flip(mean(SST(:,:,WINTER{i}),3)));  
...
for i = 1:size(Decade,2)
    nexttile
    pcolor(LON1,LAT1,flip(mean(SST(:,:,SPRING{i}),3)));  
    colormap(gca,[clri1]); shading interp;
    caxis([6 14]); box on;
    PLOT()
end
...
for i = 1:size(Decade,2)
    nexttile
    pcolor(LON1,LAT1,flip(mean(SST(:,:,SUMMER{i}),3)));  
    colormap(gca,[clri1]); shading interp;
    caxis([12 23]); box on;
    PLOT()
end
...
for i = 1:size(Decade,2)
    nexttile
    pcolor(LON1,LAT1,flip(mean(SST(:,:,FALL{i}),3)));  
    colormap(gca,[clri1]); shading interp;
    caxis([13 19]); box on;
    PLOT()
end
