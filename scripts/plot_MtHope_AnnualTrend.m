...
t=y0+dur;
MtHopeTEMPm=nan*zeros(size(MtHopeTEMPv3(:,:,1:39)));
Idx = cell(1, 2022 - 1983);

for yy=1984:2022
   ndx=find(yy==year(datet));
   tm(yy-1983)=mean(t(ndx));
   MtHopeTEMPm(:,:,yy-1983)=mean(MtHopeTEMPv3(:,:,ndx),3);
   sd_T(yy-1983) = mean(nanstd(MtHopeTEMPv3(:,:,yy-1983)));
   Idx{yy - 1983} = ndx;
end
...
save('MtHopeAnnualTrend.mat','Mean','sd','sd_T','std_T','trendLine','x','Y','years','coeffs');